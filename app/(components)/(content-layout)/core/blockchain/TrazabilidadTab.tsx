...
const TrazabilidadTab: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "Inmutable":
        return (
          <Badge bg="success">
            <i className="ri-shield-check-fill me-1"></i>Inmutable
          </Badge>
        );
      case "Modificado":
        return (
          <Badge bg="warning">
            <i className="ri-error-warning-fill me-1"></i>Modificado
          </Badge>
        );
      default:
  ...
  const getActionIcon = (action: string) => {
    switch (action) {
      ...
      case "UPDATE":
        return (
          <div className="avatar avatar-sm bg-warning text-white rounded-circle">
            <i className="ri-edit-line"></i>
          </div>
        );
      case "VERIFICATION":
        return (
          <div className="avatar avatar-sm bg-success text-white rounded-circle">
            <i className="ri-check-double-line"></i>
          </div>
        );
      default:
        return (
          <div className="avatar avatar-sm bg-secondary text-white rounded-circle">
  ...
  return (
    <>
      <Alert variant="light" className="border d-flex align-items-start gap-2 mb-4">
        ...
        <div className="fs-13">
          Selecciona un sílabo para ver su historial completo de cambios y verificar su
          integridad contra el bloque registrado en Hyperledger Fabric.
        </div>
      </Alert>
                  ...
                  filteredCourses.map((course) => (
                    ...
                    >
                      ...
                      <p className="text-muted fs-12 mb-1">
                        {course.courseCode} • {course.career}
                      </p>
            ...
            <Card className="custom-card shadow-sm border-0 h-100">
              ...
              <Card.Body className="p-4">
                ...
                <h6 id="coach-blockchain-history" className="fw-bold mb-4 text-muted">
                  <i className="ri-history-line me-2"></i>Historial de Transacciones
                </h6>
                      ...
                      <div key={index} className="d-flex mb-4 position-relative">
                        ...
                        <div className="flex-fill pb-1 border-bottom">
                          ...
                          <Badge bg="light" text="dark" className="font-monospace border">
                            TxID: {record.txId.substring(0, 32)}...
                          </Badge>
                        </div>
                      </div>
                    ))}
