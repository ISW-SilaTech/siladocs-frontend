...
const Auditoria: React.FC = () => {
  ...
  const runAudit = async () => {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        setRows((prev) =>
          prev.map((r, idx) =>
            idx === i ? { ...r, verified: result.verified, block: result.block, checking: false } : r
          )
        );
      } catch {
        setRows((prev) =>
          prev.map((r, idx) => (idx === i ? { ...r, verified: false, block: null, checking: false } : r))
        );
      }
    }
    setIsAuditing(false);
  ...
  return (
    <>
      <Seo title="Auditoría de Blockchain" />
      <Pageheader
        currentpage="Auditoría de Blockchain"
        activepage="Auditoría"
        mainpage="Blockchain"
        activepageclickable
      />
        ...
        <Card className="custom-card border-0 shadow-sm">
          <Card.Header className="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div>
              ...
              {lastAuditAt && (
                <p className="text-muted fs-12 mb-0">Última ejecución: {lastAuditAt}</p>
              )}
            </div>
